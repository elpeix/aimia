...
export async function call({ input, prompt }) {
  console.log('input', input, 'prompt', prompt)
  if (!input) throw new Error('No input provided')
  if (!prompt) throw new Error('No prompt provided')
  if (!prompt.system) throw new Error('No system prompt provided')
  if (!prompt.samples) throw new Error('No samples provided')
  ...
  prompt.samples.forEach(sample => {
    if (!sample.user || !sample.assistant) {
      throw new Error('Invalid sample provided')
    }
    messages.push({
      role: ChatCompletionRequestMessageRoleEnum.User,
      content: sample.user
